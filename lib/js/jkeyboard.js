...
;(function ( $, window, document, undefined ) {
		...
		var shift = true, capslock=true, layout = 'english', layout_id=0;
		...
		Plugin.prototype = {
				...
				events: function(){
					var letters = $(this.element).find('.letter'),
						...
						clear_key = $(this.element).find('.clearscreen'),
				...
				autocorrect (){
					    ...
					    rawFile.open("GET", "testwords.txt", true);
				...
				type (key){
					var input = this.settings.input,
						  ...
						  input_node = input.get(0),
							typing_word = val.split(" "),
							current_word = typing_word[typing_word.length -1];
					...
					if (key && key != " ") {
						...
						if (val.split(" ").length > 1) {
							input.val(val - current_word + current_word)
						}
						debugger
						input.val(val + key)
				...
				clearscreen: function(){
					var input = this.settings.input,
						...
						input.val(val.substr(0, val.length - val.length));
				},
